...
function validate_profile_image($profile_image_file, $text_frontend, $allowed_image_extensions, $allowed_image_mime_types, $maximum_image_size, $file_name, $file_error, $file_size, $file_type, $file_tmp_name) {
...
$errors [] = $text_frontend['message_profile_image_invalid_size'];
}

$image_extension = pathinfo($file_name, PATHINFO_EXTENSION);
if (! in_array(strtolower($image_extension), $allowed_image_extensions)) {
$errors [] = $text_frontend['message_profile_image_invalid_extension'];
}
if (! in_array($file_type, $allowed_image_mime_types)) {
$errors [] = $text_frontend['message_profile_image_invalid_type'];
}
...
$errors [] = $text_frontend['message_profile_image_invalid_dimension'];
}
} else {
$errors [] = $text_frontend['message_profile_image_invalid_dimension'];
}
} elseif ($file_error === UPLOAD_ERR_NO_FILE) {
$errors [] = $text_frontend['message_profile_image_no_file'];
}
}
return $errors;
}

function upload_profile_image($db, $text_frontend, $profile_image_file, $profile_user_id) {
...
$errors = validate_profile_image($profile_image_file, $text_frontend, $allowed_image_extensions, $allowed_image_mime_types, $maximum_image_size, $file_name, $file_error, $file_size, $file_type, $file_tmp_name);
...
$upload_path = profile_images_path. '/' . basename($new_profile_image_name);
